...
/**
 * Manage a Map of Resources, respectively *.properties for different Classes
 * dealing with UI. Resources are usually desired by an Application for a
 * specific Locale (for e.g. de_CH), therefore the performance may be enhanced
 * if the mapped *.properties files are cached during runtime.
 * 
...
public class ResourceManager {
	...
	private Map<String, Map<Class<?>, java.util.ResourceBundle>> resources = new HashMap<String, Map<Class<?>, java.util.ResourceBundle>>();
	...
	/**
	 * Return the resource as is in *.properties file for the given Locale (NLS
	 * translation) using a specific ClassLoader (useful for reflection
	 * frameworks such as JPF).
	 * 
	 ...
	 * 
	 * @param holder
	 *            A <holder-Class>[_<language>].properties file must exist
	 * @param locale
	 * @param propertyName
	 * @param loader
	...
	/**
	 * Return an NLS-resource for a class. Labels end according to GUI-Standards
	 * with ":", therefore this method makes sure, the colon is set, though the
	 * resource might be defined otherwise.
	 * 
	...
	/**
	 * Return NLS-Translation with "...", which is common for labels in menu's
	 * showing up a sub-window.
	 * 
	...
	/**
	 * Make sure the ":" is extracted from NLS-Translation if provided in
	 * *.properties file.
	 * 
	 * @param owner
	 * @param propertyName
	...
	/**
	 * Return the first translation found for key matching given pattern. This
	 * might be useful for generic mappings for e.g. from an Object properties
	 * to a Label corresponding in a GUI.
	 * 
	 * @param owner
	 * @param pattern
	 *            (for e.g. "LblMy[a-zA-Z0-9]*_text")
	 * @param asLabel
	 * @return
