 ...
 * 
 * Prints objects of type Document. Text attributes, including
    ...
    setting the instance of PrintUtility in any PrinterJob.
...
public class JEditorPanePrintUtility implements Printable {
  ...
  private JEditorPane jeditorPane;            //Container to hold the Document. This object will be used to lay out the Document for printing.
  private double pageEndY = 0;                //Location of the current page end.
  private double pageStartY = 0;              //Location of the current page start.
  private boolean scaleWidthToFit = true;     //boolean to allow control over whether pages too wide to fit on a page will be scaled.
...
*/
   ...
   * 
   * Optionally call #pageDialog() for any non-default page-format.
   */
  public JEditorPanePrintUtility(final String jobName) {
      super();
      
      pJob = PrinterJob.getPrinterJob();
      pJob.setJobName(jobName);
      pFormat = new PageFormat();//printJob.pageDialog(printJob.defaultPage());
  }
  /**
...
//  X
  ...
  /**
   * Called to set an HTMLDocument for printing.
   */
   ...
   * particular Document type, such as a <b>Rich Text Format</b> document. With this
  ...
  /**
   * Called to set a PlainDocument for printing.
   */
  ...
  /**
   * Recursive method which iterates through the tree structure
...
//  IV
  ...
  /**
   * Set a PlainDocument as the Document to print.
   */
  ...
  */
  private void setDocument(final String type, final Document document) {
