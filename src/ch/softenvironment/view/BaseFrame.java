...
public abstract class BaseFrame extends javax.swing.JFrame {
     ...
     * 
     * @param viewOptions
     *            Special options valid for all GUI's in an application
     * @param object
     *            Model-instances to be represented by this GUI
     */
    ...
    /**
     * Extend the given menu generically with platform dependent Look&Feel
     * styles.
     * 
    ...
    /**
     * Overwrite the #dispose() method for <b>Launcher's containing a
     * #main()</b> of any Application extending this BaseFrame. This call will
     * shutdown Application and Java virtual machine by a System#exit(). //
     * Overwrites public void dispose() { // super.dispose(); => WILL BE CALLED
     * BY #disposeApplication() disposeApplication(); }
     * 
    ...
    /**
     * Convenience method to Export table-data into a file *.csv, where a
     * File chooser allows selecting File before. The table data is exported in a
     * generic manner, say given table is exported 1:1 including Header-Data.
     * After saving the file will be opened by Excel or alternate spreadsheet
     * application registered for *.csv ending.
     * 
     * @param table
    ...
    /**
     * Display a pop-up menu. Example SearchTable to JPopupMenu 1) Connect
     * MouseReleased-Event (for Windows) and MousePressed-Event (for Unix) from
     * Panel (for e.g. JScrollPane, JTablePane, etc) to this method. 2) Connect
     * JPopupMenu's this-property visually to the MouseReleased-Event's
     * Parameter popupMenu. 3) Overwrite #adaptSelection(MouseEvent, JPopupMenu)
     * 
     * @see BaseDialog#genericPopupDisplay(java.awt.event.MouseEvent,
    ...
    /**
     * Intitalize View. Call this method in a View's standard initalize method
     * to setup your stuff.
     * 
	...
	try {
	    ...
	    BaseDialog
		    .showWarning(
			    this,
			    ResourceManager.getResource(BaseFrame.class, "CTlookAndFeel"), NlsUtils.formatMessage(ResourceManager.getResource(BaseFrame.class, "CWManagerNotAvailable"), style) + "\n" + ResourceManager.getResource(BaseFrame.class, "CWSuppressManager")); //$NON-NLS-4$//$NON-NLS-3$ //$NON-NLS-2$ //$NON-NLS-1$
	}
    }
    ...
    /**
     * Reset GUI-Components NLS-Strings, such as Component-text or
     * Component-textToolTip, usually after the Locale#getDefault() has changed.
     * Property "text" and "toolTipText" are changed in a generic, recursive
     * matter.
     * 
    ...
    /**
     * Look up a String in a properties-Resource-File, according to the
     * following rule: Component = javax.swing.JLabel Component#name =
     * LblMyLabel => entry in properties-File for "text" = LblMyLabel_text =>
     * entry in properties-File for "toolTipText" = LblMyLabel_toolTipText
     * 
     * @param component
     *            Component having the given property
     * @param property
     *            (usually "text" or "toolTipText")
     */
