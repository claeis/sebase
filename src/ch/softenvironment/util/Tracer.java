...
/**
 * This is a <b>Developer Tracing-Tool</b>, meant to trace important info a
 * Developer should be aware of <b>while developing and testing an
 * application</b>. Except of the runtime-logs the API of this Tool should not
 * be misunderstood as a User-Log Tool, rather hide log-info created by this
 * tool from user.
 * 
 * Eventually this Tool could help in a productive environment for remote
 * assistance, by starting it in a command-shell only (the application
 * parameters should forward any start-parameters to start a Tracer-Instance.
 * 
...
public class Tracer implements LogListener {
     ...
     * without using a Debugger. This method is meant for temporarily use only,
     * be means any debug-logs should be removed after testing or within
     * production releases.
     */
    ...
    /**
     * Log Developer Errors, for e.g. when a developer misunderstood the concept
     * of a called method/mechanism. If this happens, the developer is urged to
     * change this part of code immediately.
     */
    ...
    /**
     * Log developer Warnings, for e.g. if something was wrongly used by
     * Developer, but work around has taken place automatically. The developer
     * should change its code as soon as possible if this trace-log happens to
     * remain compatible for future releases.
     */
    ...
    /**
     * Log a command executed on a back-end target (for e.g. a DBMS or any other
     * Server-System).
     * 
     * @param command
     *            the remotely executed statement on Target-System
     */
    ...
    /**
     * Log Errors during runtime, by means a serious error happened, which might
     * corrupt application behaviour sooner or later.
     */
     ...
     * 
     * @param offsetToStackTrace
     *            (0 for calling method; 1 for one method earlier)
     */
     ...
     * info which might help to interpret application behaviour.
     */
    ...
    /**
     * Log Warnings during runtime, by means errors might have been happened but
     * were handled automatically. However any other follow-up errors might be
     * expected.
     */
    ...
    /**
     * Start Tracer and use Console-Error. The command line args as followed are
     * filtered out of args by this method: - "-all" - "-silent" - "-trace" -
     * "traceSQL" - "-debug")
     * 
     * @param args
     */
     ...
     * 
     * @param mode
     *            (SILENT, NORMAL, DEBUG, TRACE_BACKEND, ALL)
     */
    ...
    /**
     * Use this message to trace non-visually handled exceptions which might be
     * ignored during application run. Serious malfunctioning application
     * behaviour might be possible.
     */
    ...
    /**
     * Correct the stack-trace origin which is one method later in EhiLogger
     * convenience methods.
     * 
     * @return
     */
	...
	if (mode != ALL) {
	    if (mode == SILENT) {
		return;
	    } else if ((event.getEventKind() == LogEvent.DEBUG_TRACE) || (event.getEventKind() == LogEvent.STATE_TRACE) || (event.getEventKind() == LogEvent.UNUSUAL_STATE_TRACE) || (event.getCustomLevel() == SE_DEVELOPER_ERROR)
