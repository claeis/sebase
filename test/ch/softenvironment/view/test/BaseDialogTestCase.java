...
//	private Container mainFrame = null;
	...
	public void testShowWarning() throws Exception {
		java.util.List dialogs = JFCTestHelper.getShowingDialogs();
		assertEquals("Dialog opened", 1, dialogs.size());
		JDialog dialog = (JDialog)dialogs.iterator().next();
...
/*		JTextField inputTextField = (JTextField) JFCTestHelper.findNamedComponent("input", 0);
		assertNotNull("Es konnte keine Textfeld mit dem Namen 'input' gefunden werden.", inputTextField);
		assertEquals(inputTextField.getText(), "<Bitte geben Sie das geheime Wort ein>");
		JTextArea outputArea = (JTextArea) JFCTestHelper.findNamedComponent("output", 0);
		assertNotNull("Es konnte keine Textfeld mit dem Namen 'output' gefunden werden.", inputTextField);
*/
		
/*		inputTextField.requestFocus();
		helper.sendString(new StringEventData(this, inputTextField, "Falsches Wort"));
		helper.enterClickAndLeave(new MouseEventData(this, okButton));
		assertEquals("Die Eingabe " + inputTextField.getText() + " führte zu einem falschen Ergebnis.", "Falsch geraten, probieren Sie es mit 'Testing'.", outputArea.getText());
		// Um die Richtigkeit der Funktionalität im positiven Fall (richtige Eingabe) zu testen,
		// wird ein Tastendruck simuliert. 		
		inputTextField.requestFocus();
...
//		assertEquals("Die Eingabe " + inputTextField.getText() + " führte zu einem falschen Ergebnis.", "Testing is cool!", outputArea.getText());
	}
}